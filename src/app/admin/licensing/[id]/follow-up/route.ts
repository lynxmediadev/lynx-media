...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    await prisma.licensingRequest.update({ where: { id }, data: { nextFollowUpAt: value } });
    ...
    return NextResponse.json({ ok: false, error: "No se pudo actualizar el follow-up" }, { status: 500 });
  }
}
