...
export async function POST(req: NextRequest) {
  ...
  if (!throttle.allowed) {
    return NextResponse.redirect(redirectUrl(req, `${baseDestination}?err=rate_limited`), {
      status: 303,
    });
  }
  ...
  if (!token) {
    return NextResponse.redirect(redirectUrl(req, `${baseDestination}?err=invalid`), { status: 303 });
  }
  ...
  try {
    ...
    return NextResponse.redirect(redirectUrl(req, `${baseDestination}?err=verify_send_failed`), {
      status: 303,
    });
  }
  ...
  return NextResponse.redirect(redirectUrl(req, `${baseDestination}?ok=verify_sent${debugLink}`), {
    status: 303,
  });
}
