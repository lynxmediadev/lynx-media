...
export const ENV = {
  ...
  AUTH_EMAIL_PROVIDER: () =>
    (opt("AUTH_EMAIL_PROVIDER", process.env.AUTH_EMAIL_PROVIDER) ?? "console").toLowerCase(),
  ...
  AUTH_ENFORCE_VERIFIED_EMAIL: () =>
    opt("AUTH_ENFORCE_VERIFIED_EMAIL", process.env.AUTH_ENFORCE_VERIFIED_EMAIL) === "1",
  BREVO_API_KEY: () => opt("BREVO_API_KEY", process.env.BREVO_API_KEY),
  TURNSTILE_ENABLED: () => opt("TURNSTILE_ENABLED", process.env.TURNSTILE_ENABLED) === "1",
  TURNSTILE_SECRET_KEY: () => opt("TURNSTILE_SECRET_KEY", process.env.TURNSTILE_SECRET_KEY),
  TURNSTILE_SITE_KEY: () =>
    opt("NEXT_PUBLIC_TURNSTILE_SITE_KEY", process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY),
