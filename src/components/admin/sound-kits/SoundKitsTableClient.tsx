...
import { Archive, Check, Clock3, Copy, Filter, RefreshCcw, Search, ShieldCheck, Star, Trash2, Waves } from "lucide-react";
...
type SoundKitsTableClientProps = {
  rows: SoundKitRow[];
  filters: {
    q: string;
    status: string;
...
function statusBadge(status: SoundKitRow["status"]) {
  if (status === "PUBLISHED") {
...
function formatPrice(value: number, currency: SoundKitRow["currency"]) {
...
export function SoundKitsTableClient({ rows, filters }: SoundKitsTableClientProps) {
  const router = useRouter();
  const [selectedIds, setSelectedIds] = useState<string[]>([]);
  const [actionType, setActionType] = useState<BulkActionType>("set_status");
  const [status, setStatus] = useState<SoundKitRow["status"]>("PUBLISHED");
  ...
  async function applyBulk() {
    ...
    if (actionType === "delete") {
      const ok = window.confirm(`¿Eliminar ${selectedIds.length} sound kit(s)? Esta acción no se puede deshacer.`);
      if (!ok) return;
    }
    ...
    setFeedback(null);
    try {
      const res = await fetch("/api/admin/sound-kits/bulk", {
        method: "POST",
  ...
  async function deleteOne(id: string) {
    const ok = window.confirm("¿Eliminar sound kit? Esta acción no se puede deshacer.");
    if (!ok) return;

    try {
      const res = await fetch(`/api/admin/sound-kits/${encodeURIComponent(id)}`, { method: "DELETE" });
      if (!res.ok) throw new Error("No se pudo eliminar.");
      setFeedback({ type: "success", message: "Sound kit eliminado" });
  ...
  const desktopColumns: AdminColumnDef<SoundKitRow>[] = [
    {
      key: "select",
      label: (
        <input
          ...
          aria-label="Seleccionar todos los sound kits"
        />
      ),
      widthClassName: "w-12",
      render: (row) => (
        <input
          ...
          className="h-4 w-4 rounded border-border bg-background"
          aria-label={`Seleccionar sound kit ${row.name}`}
        />
      ),
    },
    {
      ...
      render: (row) => (
        <div className="space-y-1">
          <Link href={`/admin/sound-kits/${row.id}`} className="text-sm font-medium hover:underline">
            {row.name}
          </Link>
    ...
    { key: "estado", label: "Estado", render: (row) => statusBadge(row.status) },
    {
      key: "featured",
      label: "Destacado",
      render: (row) => (
    ...
    },
    {
      key: "price",
      label: "Precio",
      render: (row) => <span className="text-xs font-semibold">{formatPrice(row.price, row.currency)}</span>,
    },
    {
      key: "sortOrder",
      label: "Orden",
    ...
    <>
      <div className="border-b border-border px-3 py-2 sm:px-4">
        <div className="grid gap-2 xl:grid-cols-2">
          <form method="GET" action="/admin/sound-kits" className="min-w-0" onSubmit={(event) => event.preventDefault()}>
            <AdminFilterPanel
              title={
                <>
            ...
            <AdminControlsRow>
              ...
              {actionType === "set_status" ? (
                <LabeledSelect
                  ...
                  labelPosition="top"
                  value={status}
                  onChange={(event) => setStatus(event.target.value as SoundKitRow["status"])}
                  className="h-8 w-full"
                  options={[
                    { value: "DRAFT", label: "DRAFT" },
                    { value: "PUBLISHED", label: "PUBLISHED" },
      ...
      {rows.length === 0 ? (
        <AdminListEmptyState message="No hay sound kits registrados todavía." className="py-10" />
      ) : (
        <>
            ...
            {rows.map((row) => (
              ...
              >
                ...
                <div className="text-xs text-muted-foreground">
                  <Waves className="mr-1 inline h-3.5 w-3.5" />
                  {formatPrice(row.price, row.currency)} · Orden {row.sortOrder} · {formatDate(row.updatedAtIso)}
                </div>
