...
export function SoundKitsTableClient({
  rows,
  filters,
}: SoundKitsTableClientProps) {
  ...
  async function applyBulk() {
    ...
    if (actionType === "delete") {
      const ok = window.confirm(
        `¿Eliminar ${selectedIds.length} sound kit(s)? Esta acción no se puede deshacer.`,
      );
      if (!ok) return;
    }
  ...
  async function deleteOne(id: string) {
    const ok = window.confirm(
      "¿Eliminar sound kit? Esta acción no se puede deshacer.",
    );
    if (!ok) return;

    try {
      const res = await fetch(
        `/api/admin/sound-kits/${encodeURIComponent(id)}`,
        { method: "DELETE" },
      );
      ...
      render: (row) => (
        <div className="flex items-center justify-end gap-2">
          <AdminListButton asChild size="row">
            <Link href={`/admin/sound-kits/${row.id}`}>Ver</Link>
          </AdminListButton>
          <AdminListButton
            type="button"
            onClick={() => {
  ...
  return (
    <>
            ...
            <AdminControlsRow>
              ...
              {actionType === "set_status" ? (
                <LabeledSelect
                  ...
                  value={status}
                  onChange={(event) =>
                    setStatus(event.target.value as SoundKitRow["status"])
                  }
                  className="h-8 w-full"
                  options={[
                    { value: "DRAFT", label: "DRAFT" },
      ...
      {rows.length === 0 ? (
        <AdminListEmptyState
          message="No hay sound kits registrados todavía."
          className="py-10"
        />
      ) : (
        <>
            ...
            {rows.map((row) => (
              ...
              >
                ...
                <div className="text-muted-foreground text-xs">
                  ...
                  {formatPrice(row.price, row.currency)} · Orden {row.sortOrder}{" "}
                  · {formatDate(row.updatedAtIso)}
                </div>

                <div className="border-border/60 grid grid-cols-2 gap-2 border-t pt-2">
                  <AdminListButton asChild size="row">
                    <Link href={`/admin/sound-kits/${row.id}`}>
                      Ver detalle
                    </Link>
                  </AdminListButton>
                  <AdminListButton
