...
- [ `_____` ] Smoke desktop/mobile auth lifecycle.
- [ `OK` ] QA por rol (`ADMIN/STAFF/CREATOR`) sin regresiones.
- [ `_____` ] Activar `AUTH_EMAIL_PROVIDER=brevo` en staging.
- [ `_____` ] Runbook de incidentes y rollback.

### Fase 9 · Retiro legacy `admin_session` (obligatorio de cierre)
- [ `_____` ] Definir fecha oficial de corte (propuesta: **March 2, 2026**).
- [ `_____` ] Agregar telemetría de uso legacy por ruta (`legacy_admin_session_used`).
- [ `_____` ] Semana 1: desactivar emisión de cookies legacy en login (`/admin/login/submit`).
- [ `_____` ] Semana 2: bloquear aceptación de `admin_session` en middleware/guards.
- [ `_____` ] Semana 3: eliminar ramas legacy y variables no usadas.
- [ `_____` ] Actualizar documentación operativa y checklist de rollback.

---

## Smoke checklist específico (027)
- [ `_____` ] Invite enviada y recibida en bandeja real.
- [ `OK` ] Link de invite crea cuenta válida con rol correcto.
- [ `OK` ] Forgot envía reset y token expira correctamente.
- [ `OK` ] Reset actualiza password + invalida sesiones previas.
- [ `OK` ] Verify email confirma y setea `emailVerifiedAt`.
- [ `_____` ] Turnstile bloquea submit inválido.
- [ `_____` ] Turnstile permite submit válido.
- [ `OK` ] Sin filtración de tokens en logs.

---

## Checklist completo (local) · Registrar + Verificar + Usar cuenta

> Modo objetivo: **local con provider `console`** (sin Brevo aún).
...
- [ `OK` ] Validar `.env` con:
  ...
  - `TURNSTILE_ENABLED=0`
- [ `OK` ] Ejecutar `npm run auth:preflight`.
- [ `OK` ] Levantar app: `npm run dev -- --hostname 0.0.0.0 --port 3000`.
- [ `_____` ] Abrir `http://localhost:3000`.

### 1) Base admin (solo si hace falta)
- [ `OK` ] Si no puedes entrar a `/admin/login`, bootstrap admin:
  - `AUTH_BOOTSTRAP_ADMIN_EMAIL=admin@lynx.local AUTH_BOOTSTRAP_ADMIN_PASSWORD=TuClave123! npm run db:bootstrap:auth`
- [ `OK` ] Iniciar sesión admin en `/admin/login`.

### 2) Crear invitación
- [ `OK` ] Ir a `/admin/users`.
- [ `OK` ] Crear invitación con rol objetivo (ej: `CREATOR`) y email de prueba.
- [ `OK` ] Confirmar mensaje `ok=invite_created`.
- [ `OK` ] Copiar/abrir link de registro (`/auth/register?token=...`).

### 3) Registro de cuenta
- [ `OK` ] Completar form en `/auth/register` con nombre + password.
- [ `OK` ] Enviar form.
- [ `OK` ] Confirmar redirección a `/auth/verify-email?ok=sent` (o `err=send_failed`).
- [ `OK` ] En modo `console`, verificar que aparece `debugLink`.

### 4) Verificación de email
- [ `OK` ] Abrir `debugLink` (`/auth/verify-email/confirm?token=...`).
- [ `OK` ] Confirmar redirección final a dashboard por rol:
  - `CREATOR` → `/creator`
  - `ADMIN/STAFF` → `/admin`
- [ `OK` ] Confirmar en DB/UI que `emailVerifiedAt` ya no es `null`.

### 5) Uso de cuenta (post-verificación)
- [ `OK` ] Cerrar sesión.
- [ `OK` ] Login con la nueva cuenta en `/auth/login`.
- [ `OK` ] Confirmar acceso a rutas del rol:
  - `CREATOR`: `/creator/tracks`, `/creator/requests`
  - `STAFF/ADMIN`: rutas admin permitidas por guardas
- [ `OK` ] Probar una acción real del rol (ej: editar track propio para creator).

### 6) Reenvío de verificación (flujo de recuperación)
- [ `OK` ] Ir a pantalla de verificación según rol estando logueado y no verificado:
  - `CREATOR` -> `/creator/account`
  - `ADMIN/STAFF` -> `/admin/account`
- [ `OK` ] Click en reenviar (`POST /auth/verify-email/send`).
- [ `OK` ] Confirmar `ok=verify_sent` y nuevo `debugLink` en la misma pantalla de account.
- [ `OK` ] Abrir link y confirmar verificación.

### 7) Casos negativos mínimos
- [ `OK` ] Reusar token de verificación ya usado.
- [ `OK` ] Abrir token inválido/manual.
- [ `OK` ] Intentar login con password incorrecta.
- [ `OK` ] Confirmar que no hay leak de token crudo en terminal (`[redacted]` en logs console provider).

### 8) Cierre de prueba
- [ `_____` ] Registrar evidencia en `docs/debug/terminal.md`:
  - comandos ejecutados
  - resultado por paso
  - bloqueos encontrados
- [ `_____` ] Marcar en este plan qué checks quedaron OK y cuáles pendientes.
...
### A) Preparación base
- [ `OK` ] A1. Ejecutar `npm run auth:preflight`.
...
- [ `OK` ] A2. Levantar app con `npm run dev -- --hostname 0.0.0.0 --port 3000` y abrir `http://localhost:3000`.
...
- [ `OK` ] A3. Verificar que existen `AUTH_BOOTSTRAP_ADMIN_EMAIL` y `AUTH_BOOTSTRAP_ADMIN_PASSWORD` en `.env`.
...
- [ `OK` ] A4. Si no puedes entrar como admin, ejecutar bootstrap (`npm run db:bootstrap:auth` con envs del admin).
...
- [ `OK` ] B1. Crear invitación `CREATOR` desde `/admin/users`.
...
## Evidencia técnica automática (ejecución Codex)

Fecha: 2026-02-14  
Base URL: `http://127.0.0.1:3000`  
Script: `scripts/auth-smoke-027.mjs`
...
npx tsx scripts/auth-smoke-027.mjs
```
...
Checks automáticos validados en esta corrida:
...
Pendiente manual/no automatizado en esta corrida:

- `A2` (validación visual navegador desktop/mobile)
- `C5` (`AUTH_ENFORCE_VERIFIED_EMAIL=1`)
- `H2` / `H3` (Turnstile real con token válido/inválido)
- `I3` / `I4` (Brevo real + remitente + entregabilidad)
- `I5` (fallback brevo->console con servidor reiniciado en modo brevo sin API key)

Notas:

- Se corrigió condición de carrera en rate-limit (`src/lib/account-auth/rate-limit.ts`) detectada durante `C4`.
- Evidencia movida exclusivamente a este archivo (no se agregó evidencia nueva en `docs/debug/terminal.md`).
